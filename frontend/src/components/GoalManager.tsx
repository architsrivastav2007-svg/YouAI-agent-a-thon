...
export default function GoalManager() {
  ...
  const handleAddMilestone = async (goalId: string, milestoneTitle: string) => {
    ...
    try {
      const res = await api.post(`/api/goal/${goalId}/milestone`, {
      ...
      toast.error("Failed to add milestone", { id: toastId });
    }
  };
  ...
  const handleToggleMilestone = async (goalId: string, milestoneId: string) => {
    ...
    try {
      const res = await api.patch(`/api/goal/${goalId}/milestone/${milestoneId}/toggle`);
  ...
  const handleDeleteMilestone = async (goalId: string, milestoneId: string) => {
    ...
    try {
      const res = await api.delete(`/api/goal/${goalId}/milestone/${milestoneId}`);
...
function GoalCard({
  ...
  const handleProgressSubmit = () => {
    ...
    console.log("Updating progress:", { goalId: goal.id, oldProgress: goal.progress, newProgress, input: progressInput });
        ...
        <div className="space-y-4 mt-3">
          ...
          <div className="border-t border-indigo-200 pt-3">
            <Button
              ...
              className="w-full justify-between text-gray-700 hover:bg-indigo-50"
            >
            ...
            {showMilestones && (
              <div className="mt-3 space-y-2">
                {/* Add Milestone */}
                {goal.status !== "completed" && onAddMilestone && (
                  <div className="flex gap-2">
                    <Input
                      type="text"
                      ...
                      onKeyDown={(e) => {
                        if (e.key === 'Enter' && milestoneInput.trim()) {
                          onAddMilestone(goal.id || goal._id!, milestoneInput);
                          setMilestoneInput("");
                        }
                      }}
                      className="flex-1 bg-indigo-50 text-black text-xs"
                ...
                <div className="space-y-2 max-h-40 overflow-y-auto">
                  {goal.milestones && goal.milestones.length > 0 ? (
                    goal.milestones.map((milestone) => (
                      <div
                        key={milestone._id}
                        className="flex items-center gap-2 p-2 rounded bg-indigo-50 group"
                      >
                        <input
                          type="checkbox"
                          checked={milestone.completed}
                          onChange={() => onToggleMilestone?.(goal.id || goal._id!, milestone._id)}
                        ...
                        {goal.status !== "completed" && onDeleteMilestone && (
                          <button
                            onClick={() => onDeleteMilestone(goal.id || goal._id!, milestone._id)}
